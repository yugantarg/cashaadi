 ...
 * CSM — Reminder Email Monitor (Admin)

 *

 * Sales Dashboard > Reminder Emails.

 * Shows every reminder email that is scheduled, sent, cancelled or failed, and

 * gives a one-click stop. Sending stays off until the master switch is turned on.

 */



add_action( 'admin_menu', 'csm_remail_menu', 11 );

function csm_remail_menu() {

	if ( ! function_exists( 'csm_remail_table' ) ) {
...
function csm_remail_badge( $status ) {
...
function csm_remail_tile( $label, $value, $colour ) {
...
function csm_remail_handle_post() {
	...
	if ( 'pause' === $action ) {
		...
		foreach ( csm_remail_types() as $slug => $rule ) {
		...
		$notice = csm_remail_diagnose( intval( $_POST['row_id'] ) );



	} elseif ( 'send_one' === $action ) {

		$notice = csm_remail_send_one( intval( $_POST['row_id'] ) );
...
function csm_remail_render_page() {
	...
	$notice = csm_remail_handle_post();

	$counts = csm_remail_counts();

	$plan   = csm_remail_plan();

	$types  = csm_remail_types();

	$live   = csm_remail_master_on() && ! csm_remail_dry_run();
	...
	csm_remail_tile( 'Due right now', $counts['due_now'], '#1d4ed8' );

	csm_remail_tile( 'Scheduled total', $counts['pending'], '#2271b1' );

	csm_remail_tile( 'Waiting on reminder 1', isset( $counts['waiting'] ) ? $counts['waiting'] : 0, '#7c3aed' );

	csm_remail_tile( 'Deferred', $counts['deferred'], '#b45309' );

	csm_remail_tile( 'Sent', $counts['sent'], '#166534' );

	csm_remail_tile( 'Sent last hour', $counts['sent_last_hour'], '#166534' );

	csm_remail_tile( 'Cancelled', $counts['cancelled'], '#6b7280' );

	csm_remail_tile( 'Failed', $counts['failed'], '#b91c1c' );
	...
	// Read-only simulation - see csm_remail_preview() in the engine.

	// ------------------------------------------------------------------

	$pv     = csm_remail_preview( 2000 );
	...
	echo '<p><label><input type="checkbox" name="master" value="1" ' . checked( csm_remail_master_on(), true, false ) . '> <strong>Master switch: allow this queue to send email</strong></label><br>';

	echo '<span style="color:#50575e">Off means the queue keeps planning and stays visible, but nothing is delivered.</span></p>';

	echo '<p><label><input type="checkbox" name="dryrun" value="1" ' . checked( csm_remail_dry_run(), true, false ) . '> <strong>Dry run</strong> - process the queue without delivering anything</label></p>';

	echo '<p><label>Maximum sends per hour <input type="number" name="hourly_cap" min="1" max="500" value="' . esc_attr( csm_remail_hourly_cap() ) . '" style="width:90px"></label></p>';

	echo '<p><label>Only schedule members whose reminder came due in the last <input type="number" name="window" min="0" max="3650" value="' . esc_attr( csm_remail_window_days() ) . '" style="width:90px"> days</label><br>';
	...
	echo '<p><label><input type="checkbox" name="one_per_day" value="1" ' . checked( csm_remail_one_per_day(), true, false ) . '> <strong>Never send one member more than one reminder in 24 hours</strong></label><br>';
	...
	if ( ! $rows ) {
		...
		foreach ( $rows as $r ) {
			...
			echo '<td><a href="' . esc_url( get_edit_user_link( $r->user_id ) ) . '">' . esc_html( csm_remail_display_name( $r->user_id ) ) . '</a></td>';
			...
			echo '<td>' . csm_remail_badge( $r->status ) . '</td>';
	...
	if ( ! $hist ) {
		...
		foreach ( $hist as $r ) {
			...
			echo '<td>' . esc_html( csm_remail_display_name( $r->user_id ) ) . '</td>';
