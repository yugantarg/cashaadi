 ...
 * CSM — Reminder Email Queue (Engine)

 *

 * Plans the four BuddyPress reminder emails into a visible queue table and only

 * sends them when sending is explicitly switched on in the Reminder Emails screen.

 *

 * Default state: planning ON, sending OFF (csm_remail_master = '0').

 * Kill switch: set option csm_remail_master to '0' and nothing leaves the server.

 */



if ( ! defined( 'CSM_REMAIL_DB_VER' ) ) {

	define( 'CSM_REMAIL_DB_VER', '1' );

}



function csm_remail_table() {
...
function csm_remail_plan() {

	$saved = get_option( 'csm_remail_plan', array() );

	$plan  = csm_remail_types();
...
function csm_remail_one_per_day() {
...
function csm_remail_member_sent_within( $uid, $hours = 24 ) {
...
function csm_remail_hourly_cap() {
...
function csm_remail_window_days() {
...
function csm_remail_install() {

	global $wpdb;

	if ( CSM_REMAIL_DB_VER === get_option( 'csm_remail_db_ver' ) ) {
 ...
 KEY status_sched (status,scheduled_for)
	...
	update_option( 'csm_remail_db_ver', CSM_REMAIL_DB_VER );

}

add_action( 'admin_init', 'csm_remail_install', 5 );
...
function csm_remail_migrate_sequence() {
...
add_action( 'admin_init', 'csm_remail_migrate_sequence', 6 );



function csm_remail_family_for_label( $label ) {
...
function csm_remail_display_name( $uid ) {
 ...
 * Tokens handed to every reminder email.

 * BuddyPress only substitutes tokens the caller supplies, so anything the

 * templates reference has to be listed here or it renders literally.
...
function csm_remail_activation_key( $uid ) {
...
function csm_remail_activation_url( $uid ) {

	$key = csm_remail_activation_key( $uid );
...
function csm_remail_profile_edit_url( $uid ) {
...
function csm_remail_tokens( $uid, $label = '' ) {

	$name = csm_remail_display_name( $uid );
	...
	return array(
		...
		'profile.edit.url'   => csm_remail_profile_edit_url( $uid ),

		'activate.fresh.url' => csm_remail_activation_url( $uid ),
...
function csm_remail_replan( $limit = 1000 ) {
	...
	$plan   = csm_remail_plan();

	$window = csm_remail_window_days();
	...
	foreach ( $users as $u ) {
		...
		$family = csm_remail_family_for_label( $label );
...
function csm_remail_sent_last_hour() {
...
function csm_remail_due_rows( $limit = 25 ) {
...
function csm_remail_counts() {
	...
	$out['sent_last_hour'] = csm_remail_sent_last_hour();
...
function csm_remail_process() {
	...
	if ( ! csm_remail_master_on() ) {

		$out['due']  = count( csm_remail_due_rows( 500 ) );
	...
	$live        = ! csm_remail_dry_run();

	$out['mode'] = $live ? 'live' : 'dry-run';

	$room        = csm_remail_hourly_cap() - csm_remail_sent_last_hour();
	...
	$rows  = csm_remail_due_rows( $batch );
	...
	foreach ( $rows as $r ) {
		...
		$now_f = csm_remail_family_for_label( $label );
		...
		if ( 'activation' === $want && '' === csm_remail_activation_url( $r->user_id ) ) {
		...
		if ( csm_remail_one_per_day() && csm_remail_member_sent_within( $r->user_id, 24 ) ) {
		...
		if ( function_exists( 'bp_send_email' ) ) {
			...
			$sent = bp_send_email( $r->email_type, intval( $r->user_id ), array(
				...
				'tokens' => csm_remail_tokens( $r->user_id, $label ),
...
add_action( 'csm_remail_cron', 'csm_remail_cron_tick' );



function csm_remail_schedule() {
...
add_action( 'init', 'csm_remail_schedule' );



// Capture the last transport-level mail failure so a test send can report why.

add_action( 'wp_mail_failed', 'csm_remail_capture_mail_error' );

function csm_remail_capture_mail_error( $e ) {
...
function csm_remail_send_one( $id ) {
...
function csm_remail_diagnose( $id ) {
	...
	foreach ( $pairs as $tag => $type ) {
		...
		$email->set_tokens( csm_remail_tokens( $r->user_id, 'Profile pending' ) );
 ...
 * Read-only simulation: exactly what csm_remail_process() would do if the

 * master switch were on and dry run off. Sends nothing, writes nothing.

 * Mirrors the sender's gates in the same order.

 */

function csm_remail_preview( $limit = 1500 ) {
	...
	$out = array(
		...
		'per_run'   => min( 25, csm_remail_hourly_cap() ),

		'cap'       => csm_remail_hourly_cap(),
	...
	foreach ( $rows as $r ) {
		...
		$item = array(
			...
			'name'    => csm_remail_display_name( $r->user_id ),
		...
		if ( csm_remail_one_per_day() && ( isset( $seen[ $r->user_id ] ) || csm_remail_member_sent_within( $r->user_id, 24 ) ) ) {
	...
	if ( $wait ) {
		...
		foreach ( $wait as $w ) {
			...
			$out['waiting'][] = array(
				...
				'name'    => csm_remail_display_name( $w->user_id ),
